...
* Für dieses Projekt wird die MariaDB (Version 10.5.15) genutzt, die in einer Debian 5.10 VM gehostet wird.
...
Das Programm wurde in Windows (10 und 11) geschrieben.
...
Das Programm läuft auf einen Node-server (Version 14+).
...
Erwartet war, dass sie einen Wert liefern, dabei war der return Wert aber `undefined`.
Gelöst hat man das Problem, mithilfe von `Promises` und `Callbacks`.

Ein weiteres Problem war, dass bei der Konvertierung der Datenbankabfragen zu JSON Objekte, vor jedem Objekt
...
Diese formattiert den eingegebenen String zu ein Json Objekt und dann wieder zu String.
